...
class ValidationEngine:
                ...
                if line_content and line_content[-1] == ")":
                    ...
                    if open_braces > close_braces:
                        ...
                        if last_paren_idx > 0:
                            ...
                            if not (
                                "$(" in before_paren
                                and before_paren.count("$(") > before_paren.count(")")
                            ):
                                result.add_issue(
                                    ValidationIssue(
                                        severity=Severity.CRITICAL,
    ...
    def _validate_cross_references(
        self, lines: List[str], result: ValidationResult, file_path: str
    ):
    ...
    def validate_startup_script(self, file_path: str) -> ValidationResult:
        ...
        dbload_pattern = re.compile(
            r'dbLoadRecords\s*\(\s*"([^"]+)"\s*(?:,\s*"([^"]+)")?\s*\)'
        )
        envset_pattern = re.compile(
            r'epicsEnvSet\s*\(\s*"([^"]+)"\s*,\s*"([^"]+)"\s*\)'
        )
    ...
    def generate_diff(self, original_lines: List[str], fixed_lines: List[str]) -> str:
                    ...
                    if issue.current_value is not None:
                        ...
                        if issue.current_value == "" and issue.suggested_value == '""':
                            ...
                            fixed_line = line.replace(
                                issue.current_value, issue.suggested_value, 1
